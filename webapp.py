from flask
import Flask, redirect, url_for, session, request, jsonify, flash
...
# This code originally from https: //github.com/lepture/flask-oauthlib/blob/master/example/github.py
    #Edited by P.Conrad
for SPIS 2016 to add getting Client Id and Secret from# environment variables, so that this will work on Heroku.#Edited by S.Adams
for Designing Software
for the Web to add comments and remove flash messaging

app = Flask(__name__)

app.debug = True# Change this to False
for production
...
github = oauth.remote_app(
    ...
    URL
    for github 's OAuth login
)

# context processors run before template are rendered and add variable(s) to the template# context processors must
return a dictionary@ app.context_processor
def inject_logged_in():
    return {
        "logged_in": ('github_token' in session)
    }
...
message = 'Access denied: reason=' + request.args['error'] + ' error=' + request.args['error_description'] + ' full=' + pprint.pformat(request.args)
    ...
    try: #save user data and set log in message
session['github_token'] = (resp['access_token'], '')
session['user_data'] = github.get('user').data
message = "Your were successfully logged in as " + session['user_data']['login']
except: #clear the session and give error message
session.clear()
...
# the tokengetter is automatically called to check who is logged in @github.tokengetter
